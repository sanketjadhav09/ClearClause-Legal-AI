...
# --- Warm-up Model (shared for reuse) ---
shared_model = genai.GenerativeModel('gemini-2.5-pro')
try:
    print("Performing a warm-up call to the Gemini API...")
    shared_model.generate_content("Hello")
    print("Warm-up call successful.")
except Exception as e:
    print(f"Warm-up call failed, but the app will continue. Error: {e}")
...
def get_summary_chain():
    model = ChatGoogleGenerativeAI(model="gemini-2.5-pro", temperature=0.2, google_api_key=api_key)
...
def handle_general_qna(user_question):
    prompt = f"You are ClearClause, a helpful Legal AI Assistant. Answer the following general legal question: {user_question}"
    try:
...
def generate_summary(custom_instruction):
    ...
    with st.spinner("Generating detailed summary..."):
        ...
        refine_prompt = f"Given the following summary, refine it to strictly follow this instruction: '{custom_instruction}'.\n\nSUMMARY:\n{initial_summary}"
        final_summary = safe_extract_text(shared_model.generate_content(refine_prompt))
...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        ...
        help="You can upload one or more .pdf or .docx files."
    )
    ...
    if st.session_state.vector_store:
        ...
        custom_instruction = st.text_input(
            "Custom Summary Instruction (Optional):", 
            "Provide a standard one-paragraph summary."
        )
